...
const date_input = document.getElementById("incomeDate");
const category_input = document.getElementById("incomeCategory");
const amount_input = document.getElementById("incomeAmount");
...
function saveToLocalStorage(data) {
    ...
    saveTotalIncome(data.datasets)
    saveIncomeArray(data.datasets)
    console.log('Data saved to LocalStorage with key incomeData');
    calculateAndSaveSavings();
}

function loadFromLocalStorage() {
    storedData = JSON.parse(localStorage.getItem("incomeData"));
    if (storedData) {
        data = storedData;
    }
    console.log('Data loaded from LocalStorage with key incomeData');

}

function saveTotalIncome(datasets) {
    let totalIncome = datasets.reduce((sum, dataset) => {
...
function saveIncomeArray(datasets) {
    ...
    localStorage.setItem("incomeArray", JSON.stringify(incomeDataset));

    console.log("Income Array Saved:", incomeDataset);
}
        ...
        options: {
            ...
            onClick: function(event, elements) {
                if (elements.length > 0) {
                    ...
                    document.getElementById("incomeDate").value = parseMonthYear(this.data.labels[dataIndex]);
                    document.getElementById("incomeCategory").value = this.data.datasets[datasetIndex].label;
                    document.getElementById("incomeAmount").value=this.data.datasets[datasetIndex].data[dataIndex];
                }
            },
...
document.getElementById("income-add").addEventListener('click', (e) => {
    e.preventDefault();
...
document.getElementById("income-edit").addEventListener('click', (e) => {
    e.preventDefault();
    Swal.fire({
        title: "Edit Data",
...
document.getElementById("income-save").addEventListener('click', (e) => {
    e.preventDefault();
...
document.getElementById("income-cancel").addEventListener('click', (e) => {
    e.preventDefault();
    disableEditing();
    clearFields();
})

document.getElementById("income-clear").addEventListener('click', (e) => {
    e.preventDefault();
    clearFields();
    disableEditing();
})
...
function openModal() {
    ...
    let storedData = JSON.parse(localStorage.getItem("incomeData")) || { labels: [], datasets: [] };
    ...
    if (Object.keys(groupedData).length === 0) {
        incomeContainer.innerHTML = `<p id="noDataMessage">No data available</p>`;
    } else {
        ...
        sortedMonths.forEach(month => {
            ...
            incomeContainer.appendChild(monthGroup);
        });
    }
}
...
document.getElementById("reset-history").addEventListener('click', () => {
    Swal.fire({
        title: "Are you sure?",
        text: "This will remove all income data!",
        icon: "warning",
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            Swal.fire("Deleted!", "All income data has been deleted.", "success").then(() => {
                data = []
                createChart('income-chart', data);
                calculateAndSaveSavings();
            });
